 ...
 *
 * <p>WHY THIS WAS WRITTEN: - Builds a custom framework that validates record constraints using
 * Reflection and custom annotations.
 *
 * <p>KEY LESSONS: - Reflection can read field annotations at runtime. - Useful for building
 * validation logic similar to Jakarta Bean Validation.
 */
...
public class ValidationFrameworkChallenge {
     ...
     *
     * <p>1. If the object's class is not annotated with `@Validate`, throw an
     * `IllegalArgumentException("Class must be annotated with @Validate")`. 2. Inspect all declared
     * fields of the object (including private ones!). 3. For each field: - If annotated with
     * `@NotBlank`: Ensure the value is a non-null, non-empty, non-whitespace String. If validation
     * fails, add an error formatted as: "fieldName: message" - If annotated with `@Min`: Ensure the
     * value (which is a Number/integer) is >= the annotation's value. If validation fails, add an
     * error formatted as: "fieldName: message value" (e.g. "age: must be at least 18") - If
     * annotated with `@Email`: Ensure the value is a String containing "@". If validation fails or
     * is null, add an error formatted as: "fieldName: message" 4. Return a List containing all
     * validation error strings. Return an empty list if there are no errors.
     *
        ...
        for (Field field : fields) {
            ...
            if (field.isAnnotationPresent(Min.class)) {
                ...
                if (rawValue == null
                        ...
                        || ((Number) rawValue).longValue() < min.value()) {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            boolean hasUsernameError =
                    errors.stream().anyMatch(e -> e.contains("username: Username cannot be empty"));
            boolean hasAgeError =
                    errors.stream().anyMatch(e -> e.contains("age: Age must be at least 18"));
            boolean hasEmailError =
                    errors.stream().anyMatch(e -> e.contains("email: Email must be valid"));
        ...
        try {
            ...
            System.err.println(
                    "Test 3 Failed: Expected IllegalArgumentException but execution succeeded");
