 ...
 *
 * <p>WHY THIS WAS WRITTEN: - Demonstrates how to delegate behavior to enum constants using abstract
 * methods while correctly handling custom checked exceptions in the signature.
 *
 * <p>KEY LESSONS: - Polymorphic enums allow you to define constant-specific behaviors instead of
 * large switch-cases. - Abstract methods declared on the enum class can throw checked exceptions
 * that each constant must implement or declare.
 */
import java.util.*;
...
public class ApplyExample {
    ...
    static enum Operation {
        ...
        public static Optional<Operation> fromString(String s) {
            ...
            return Stream.of(values()).filter(op -> op.name().equalsIgnoreCase(s)).findFirst();
        }
    }
}
