 ...
 *
 * <p>WHY THIS WAS WRITTEN: - Refactors report exporting into a stateless service pattern using
 * records.
 *
 * <p>KEY LESSONS: - Separate state (records) from behaviors (stateless service classes). - Method
 * arguments should pass dynamic state; constructors should inject stateless dependencies.
 */
...
public class ReportGeneratorRefactor {
    ...
    public void generateReport(List<Transaction> txs, String outputPath) {
        ...
        System.out.println(
                "Total Sales: " + alltxns.stream().mapToDouble(Transaction::amount).sum());
        System.out.println(
                "High Value Count: " + alltxns.stream().filter(txn -> txn.amount() > 1000).count());
    ...
    static class ReportGeneratorOld {
    ...
    // Nested helper class to prevent namespace conflicts
    static record Transaction(String id, double amount, String status) {}
    ...
    // Nested helper class to prevent namespace conflicts
    static class CsvWriteFile implements WriteFile {

        @Override
        public void write(String outputPath, List<Transaction> highValueTxs) {

            try (var writer = new FileWriter(outputPath, java.nio.charset.StandardCharsets.UTF_8)) {
