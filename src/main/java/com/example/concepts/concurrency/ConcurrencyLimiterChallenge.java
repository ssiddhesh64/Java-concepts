 ...
 *
 * <p>WHY THIS WAS WRITTEN: - A challenge to restrict concurrent execution of a slow method using a
 * Semaphore.
 *
 * <p>KEY LESSONS: - Semaphores are ideal for restricting the maximum number of concurrent threads
 * entering a block of code. - Always call semaphore.release() in a finally block to prevent
...
public class ConcurrencyLimiterChallenge {
    ...
    public static Supplier<CompletableFuture<String>> createTask(
     ...
     *
     * <p>1. Start at most `maxConcurrent` tasks initially. 2. As soon as any running task completes
     * successfully, start the next pending task. 3. Collect the results of all tasks and return
     * them in a list that preserves the original index order. 4. If any task fails, the returned
     * future should immediately complete exceptionally with that exception. Further pending tasks
     * should not be started. 5. The entire implementation must be non-blocking (do not call
     * `.get()`, `.join()`, or block a thread). 6. If the input list is null or empty, return a
     * completed future with an empty list.
     */
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            System.out.println(
                    "Max observed concurrent tasks running: " + maxObservedConcurrency.get());
            System.out.println("Total duration: " + duration + " ms");

            // Assertions
            boolean orderPreserved =
                    List.of("Result_A", "Result_B", "Result_C", "Result_D", "Result_E")
                            .equals(results);
        ...
        List<Supplier<CompletableFuture<String>>> failingTasks =
                List.of(
