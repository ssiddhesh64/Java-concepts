 ...
 *
 * <p>WHY THIS WAS WRITTEN: - Demonstrates modern Java syntax including sealed interfaces, records,
 * and pattern matching.
 *
 * <p>KEY LESSONS: - Sealed classes/interfaces restrict subclass inheritance. - Records are concise
 * data carriers. - Pattern matching instanceof automatically binds variables upon type check.
 */
import java.util.*;
...
public class Streams {
    ...
    public static String cleanAndConcatenate(List<String> strs) {
        ...
        return Optional.ofNullable(strs).orElse(List.of()).stream()
                .filter(str -> str != null && !str.isBlank())
                .map(String::toUpperCase)
                .collect(Collectors.joining(", "));
    }

    public static <T> void filterAndCopy(
            List<? extends T> src, List<? super T> dest, Predicate<? super T> pred) {
        // antipattern
    ...
    public static void main(String[] args) {
        ...
        TriFunction<String, String, String, String> triFunction =
                (a, b, c) -> String.join(" ", List.of(a, b, c));

        TriFunction<String, String, String, String> betteTriFunction =
                (a, b, c) ->
                        String.join(
                                " ",
                                Stream.of(a, b, c).filter(Objects::nonNull).toArray(String[]::new));
