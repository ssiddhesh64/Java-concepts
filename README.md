...
| [TokenBucketRateLimiter.java](file:///Users/siddheshsawant/Desktop/java/TokenBucketRateLimiter.java) | Lock-Free Rate Limiter | High-performance API rate limiting. | Convert synchronization locks to lock-free CAS loops using `AtomicReference` of immutable state records. Use precise additions to prevent timeline/fractional token drift. |
| [ParallelDataScraper.java](file:///Users/siddheshsawant/Desktop/java/ParallelDataScraper.java) | ForkJoinPool Thread Starvation | Avoid blocking calls in parallel streams. | Blocking operations inside parallel streams exhaust the shared `ForkJoinPool.commonPool()`, starving other tasks JVM-wide. Solve by using native non-blocking APIs (like `HttpClient.sendAsync()`) or custom executors. |
| [SimpleConnectionPool.java](file:///Users/siddheshsawant/Desktop/java/SimpleConnectionPool.java) | Thread-Safe Connection Pool | Avoid sleep loops inside synchronized locks. | Calling `Thread.sleep()` inside synchronized blocks blocks other threads from releasing resources, creating deadlocks. Solve by replacing list queues and sleep monitors with a thread-safe `BlockingQueue`. |
| [MemoizerCache.java](file:///Users/siddheshsawant/Desktop/java/MemoizerCache.java) | Cache Stampede Memoizer | Prevent double-computation (Cache Stampede). | Atomically map `CompletableFuture` computations to missing keys using `computeIfAbsent()` to avoid map bucket locking. Always use `remove(key, future)` on failure to prevent stale cache eviction race conditions. |
| [SilentAuditLogger.java](file:///Users/siddheshsawant/Desktop/java/SilentAuditLogger.java) | Async Exception Swallowing | Expose silent background future failures. | CompletableFuture background tasks swallow exceptions silently if the future is never read via get()/join() or registered with handler stages. Solve by always appending `.exceptionally()` or `.handle()` handlers. |
| [SimpleConcurrentStack.java](file:///Users/siddheshsawant/Desktop/java/SimpleConcurrentStack.java) | Lock-Free Treiber Stack | High-throughput concurrent stack (LIFO). | Convert thread-blocking synchronization into lock-free Treiber stacks using `AtomicReference` head nodes and conditional `do-while` CAS retry loops. Re-read node values inside loops to prevent stale infinite busy-spin loops. |
| [ReadHeavyCache.java](file:///Users/siddheshsawant/Desktop/java/ReadHeavyCache.java) | StampedLock Optimistic Reading | Prevent writer thread starvation. | ReentrantReadWriteLock starves writer threads under read-heavy workloads since write locks block on active readers. Solve by utilizing Java 8 StampedLock optimistic reads, checking validity, and falling back to read-locks only if written. |
...
| [TenantContext.java](file:///Users/siddheshsawant/Desktop/java/TenantContext.java) | ThreadLocal Memory & Data Leaks | Safe ThreadLocal context management. | Storing data in `ThreadLocal` on pooled threads leaks data across concurrent requests. Solve by returning `AutoCloseable` wrappers that enforce `ThreadLocal.remove()` using compile-time try-with-resources. |
