 ...
 *
 * <p>WHY THIS WAS WRITTEN: - Shows how to implement a retry mechanism for async tasks without
 * blocking threads using a ScheduledExecutorService.
 *
 * <p>KEY LESSONS: - Do not use Thread.sleep() inside async tasks as it blocks carrier/pool threads.
 * - Use ScheduledExecutorService.schedule() to trigger delayed attempts asynchronously. - Chain
 * delayed attempts recursively via CompletableFuture pipelines.
 */
...
public class AsyncRetryChallenge {
     ...
     *
     * <p>1. Execute the future provided by `taskSupplier`. 2. If it succeeds, return the result. 3.
     * If it fails and `maxAttempts` has not been reached, wait for `delayMs` (using the provided
     * `scheduler` non-blockingly) and try again. 4. If all attempts fail, complete the returned
     * future exceptionally with the exception from the final attempt. 5. The implementation must be
     * non-blocking (do not use Thread.sleep() or block on futures).
     *
    ...
    public static <T> void attempt(
            Supplier<CompletableFuture<T>> taskSupplier,
            int currentAttempt,
            int maxAttempts,
            long delayMs,
            ScheduledExecutorService scheduler,
            CompletableFuture<T> ans) {
            ...
            if (future == null) {
                future =
                        CompletableFuture.failedFuture(
                                new NullPointerException("Supplier returned a null future"));
            }
        } catch (Throwable ex) {
        ...
        try {
            ...
            try {
                ...
                if (service2.getAttempts() == 2) {
                    ...
                    System.err.println(
                            "FAILURE: Expected exactly 2 attempts, but got "
                                    + service2.getAttempts());
                }
            }
