 ...
 *
 * <p>WHY THIS WAS WRITTEN: - A challenge to coordinate parallel and sequential asynchronous tasks
 * with dependencies, ensuring correct handling of partial failures.
 *
 * <p>KEY LESSONS: - Use CompletableFuture.allOf() to run unrelated asynchronous tasks in parallel.
 * - Use thenCompose() to chain dependent async tasks without nesting futures. - Ensure exception
 * propagation handles failures gracefully without hanging.
 */
...
public class AsyncPipelineChallenge {
    ...
    public static CompletableFuture<Void> sendNotification(User user, PaymentResult result) {
        return CompletableFuture.runAsync(
                () -> {
                    ...
                    System.out.println(
                            "[SERVICE] Notification sent successfully to " + user.email());
                });
    }
     ...
     *
     * <p>1. Fetch the user profile and check product inventory in PARALLEL. 2. If the product is
     * out of stock (isAvailable is false), immediately fail the pipeline with an
     * IllegalStateException("Product out of stock"). 3. Once both tasks complete successfully,
     * process the payment for the given amount. 4. If the payment fails (success is false), fail
     * the pipeline with an IllegalStateException containing the payment error message. 5. If the
     * payment is successful, send a notification to the user. 6. Resilience: If the notification
     * service fails (throws an exception), the overall pipeline should STILL complete successfully
     * with the PaymentResult (graceful degradation). 7. Timeout: The entire process must complete
     * within 2 seconds. If it takes longer, it must fail with a TimeoutException.
     *
     * @return a CompletableFuture containing the PaymentResult on success
     * @throws ExecutionException
     * @throws InterruptedException
     */
    public static CompletableFuture<PaymentResult> processOrder(
            String userId, String productId, double amount) {
    ...
    public static void main(String[] args) {
        ...
        try {
            PaymentResult res =
                    processOrder("user_01", "product_01", 50.0).get(3, TimeUnit.SECONDS);
        ...
        try {
            PaymentResult res =
                    processOrder("trigger_notification_error", "product_01", 50.0)
    ...
    static record User(String userId, String email, double balance) {}

    // Nested helper class to prevent namespace conflicts
    static record Inventory(String productId, boolean isAvailable) {}

    // Nested helper class to prevent namespace conflicts
    static record PaymentResult(String transactionId, boolean success, String errorMessage) {}
}
