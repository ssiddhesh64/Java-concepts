 ...
 *
 * <p>WHY THIS WAS WRITTEN: - The refactored version of OrderProcessor showing clean decoupling,
 * interfaces, and polymorphic enums.
 *
 * <p>KEY LESSONS: - Inject interfaces for external tasks (DIP). - Polymorphic enums delegate
 * specific logic (like discounts) cleanly.
 */
...
public class OrderProcessor2 {
    ...
    public OrderProcessor2(
            DatabaseOrderRepository databaseRepository, NotificationService notificationService) {
    ...
    public void process(Order order) {
        ...
        //     System.out.println("Sending email to " + email + ": Your order of $" + total + " has
        // been processed.");
        // }
    ...
    //             String sql = "INSERT INTO orders (id, total) VALUES (" + order.getId() + ", " +
    // total + ")";
    //             System.out.println("Executing SQL in database: " + sql);
    ...
    //             if (email != null && email.contains("@")) {
    //                 System.out.println("Sending email to " + email + ": Your order of $" + total
    // + " has been processed.");
    //             }
    //         } else {
    //             throw new IllegalArgumentException("No items in order");
    ...
    static interface DatabaseOrderRepository {
        void save(Order order, double total);
    }

    // Nested helper class to prevent namespace conflicts
    static class MongoDBRepo implements DatabaseOrderRepository {
    ...
    // Nested helper class to prevent namespace conflicts
    static class PostgresRepo implements DatabaseOrderRepository {
    ...
    // Nested helper class to prevent namespace conflicts
    static record Order(String id, List<Item> items, OrderType type, String email) {
    ...
    // Nested helper class to prevent namespace conflicts
    static record Item(double price, int quantity) {
        public Item {
            if (price < 0) {
